...
void main() {
  ...
  test('catalogs from every daemon flatten into sorted projects', () {
    ...
    expect(
      ...
      <String>['Alpha daemon/Alpha', 'Alpha daemon/Zulu', 'Beta daemon/Tinest'],
    );
  ...
  testWidgets('new workspace follows the application UI density boundary', (
    tester,
  ) async {
    ...
    final agent = find.byKey(const ValueKey<String>('session-composer-agent'));
    final model = find.byKey(const ValueKey<String>('session-composer-model'));
  ...
  testWidgets('the local checkout starts a session without creating one', (
    tester,
  ) async {
    await tester.binding.setSurfaceSize(const Size(1100, 900));
    addTearDown(() => tester.binding.setSurfaceSize(null));
  ...
  testWidgets('a failed worktree keeps the user in the composer', (
    tester,
  ) async {
    await tester.binding.setSurfaceSize(const Size(1100, 900));
    addTearDown(() => tester.binding.setSurfaceSize(null));
  ...
  testWidgets('a failed setup hook reports output without starting a session', (
    tester,
  ) async {
    await tester.binding.setSurfaceSize(const Size(1100, 900));
    addTearDown(() => tester.binding.setSurfaceSize(null));
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(tester.widget<Text>(triggerText).overflow, TextOverflow.ellipsis);
      expect(
  ...
  testWidgets('a chip menu opens against the chip, not the pane corner', (
    tester,
  ) async {
    await tester.binding.setSurfaceSize(const Size(1100, 900));
    addTearDown(() => tester.binding.setSurfaceSize(null));
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.trackpadFling(optionsScroll, const Offset(0, -1000), 1000);
      await tester.pumpAndSettle();
  ...
  testWidgets('activating a hovered project chip dismisses its tooltip', (
    tester,
  ) async {
    await tester.binding.setSurfaceSize(const Size(1100, 900));
    addTearDown(() => tester.binding.setSurfaceSize(null));
  ...
  testWidgets('without a project the composer explains what to do first', (
    tester,
  ) async {
    await tester.binding.setSurfaceSize(const Size(1100, 900));
    addTearDown(() => tester.binding.setSurfaceSize(null));
  ...
  testWidgets('a directory project without its checkout cannot submit', (
    tester,
  ) async {
    await tester.binding.setSurfaceSize(const Size(1100, 900));
    addTearDown(() => tester.binding.setSurfaceSize(null));
  ...
  testWidgets('the project selector is disabled while no daemon is connected', (
    tester,
  ) async {
    await tester.binding.setSurfaceSize(const Size(1100, 900));
    addTearDown(() => tester.binding.setSurfaceSize(null));
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        find.byKey(const ValueKey<String>('new-workspace-targets-loading')),
        findsOneWidget,
      );
  ...
  testWidgets('the home composer offers the commands it can carry out', (
    tester,
  ) async {
    await tester.binding.setSurfaceSize(const Size(1100, 900));
    addTearDown(() => tester.binding.setSurfaceSize(null));
  ...
  testWidgets('a removed host command is submitted as ordinary prompt text', (
    tester,
  ) async {
  ...
  testWidgets('a settings command from the home composer opens that screen', (
    tester,
  ) async {
    await tester.binding.setSurfaceSize(const Size(1100, 900));
    addTearDown(() => tester.binding.setSurfaceSize(null));
    ...
    expect(router.routeInformationProvider.value.uri.path, '/settings/agents');
    expect(api.createdSessions, isEmpty);
  }, tags: const <String>['feature_test__composer_slash_command__widget']);
    ...
    ProviderScope(
      ...
      child: MaterialApp.router(
        ...
        builder: (context, child) =>
            TinestUiDensity(child: TRTooltipProvider(child: child!)),
      ),
    ),
  );
  if (settle) {
