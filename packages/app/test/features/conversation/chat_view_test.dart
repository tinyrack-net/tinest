...
void main() {
  ...
  testWidgets('tool calls collapse to a CLI line instead of raw JSON', (
    tester,
  ) async {
  ...
  testWidgets('a missing command UI snapshot falls back to a raw disclosure', (
    tester,
  ) async {
    ...
    await tester.pumpAndSettle();
    expect(clipboard.single, contains(r'"output": "All tests passed!\ndone"'));
  ...
  testWidgets('a missing patch UI snapshot falls back to immutable raw data', (
    tester,
  ) async {
    await pump(tester, <TimelineEventDto>[
      event('tool.requested', <String, dynamic>{
  ...
  testWidgets('running shimmer becomes static when reduced motion is enabled', (
    tester,
  ) async {
    await pump(
      tester,
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(collapsedSemantics.flagsCollection.isExpanded, Tristate.isFalse);
      await tester.tap(find.byType(ChatReasoningCard));
      await tester.pumpAndSettle();
  ...
  testWidgets('running state participates in the timeline layout', (
    tester,
  ) async {
    final events = <TimelineEventDto>[
          ...
          'document': <String, dynamic>{
            ...
            'root': <String, dynamic>{
              ...
              'children': <Map<String, dynamic>>[
                <String, dynamic>{'type': 'text', 'text': '첫 줄에 맞춰요'},
              ],
            },
          },
        }),
  ...
  testWidgets('assistant links open only browser-safe schemes', (tester) async {
  ...
  testWidgets('expanded cards keep their state when a new event arrives', (
    tester,
  ) async {
  ...
  testWidgets('an answered question renders as prose, marking typed answers', (
    tester,
  ) async {
    await pump(tester, <TimelineEventDto>[
      event('tool.requested', <String, dynamic>{
  ...
  testWidgets('a running sleep counts down and settles when it ends', (
    tester,
  ) async {
  ...
  testWidgets('a sleep without a usable duration stays an ordinary tool row', (
    tester,
  ) async {
    await pump(tester, <TimelineEventDto>[
      event('tool.requested', <String, dynamic>{
    ...
    expect(find.byKey(const ValueKey<String>('chat-sleep-card')), findsNothing);
  ...
  testWidgets('a rejected context reset stays visible as a tool row', (
    tester,
  ) async {
    await pump(tester, <TimelineEventDto>[
      event('tool.requested', <String, dynamic>{
  ...
  testWidgets('withheld tools are announced so the user knows they exist', (
    tester,
  ) async {
    await pump(tester, <TimelineEventDto>[
      event('tools.deferred', <String, dynamic>{'count': 12, 'surfaced': 0}),
    ]);
    await tester.pumpAndSettle();
  ...
  testWidgets('nothing withheld shows no notice', (tester) async {
    await pump(tester, <TimelineEventDto>[
  ...
  testWidgets('token usage reads as labelled counters, not raw provider keys', (
    tester,
  ) async {
    await pump(tester, <TimelineEventDto>[
  ...
  testWidgets('a measured response reports its output rate on the usage line', (
    tester,
  ) async {
    await pump(tester, <TimelineEventDto>[
      event('model.usage', <String, dynamic>{
  ...
  testWidgets('a usage event without a measured span reports counters only', (
    tester,
  ) async {
    await pump(tester, <TimelineEventDto>[
      event('model.usage', <String, dynamic>{
  ...
  testWidgets('a usage event with nothing to report renders nothing', (
    tester,
  ) async {
    await pump(tester, <TimelineEventDto>[
    ...
    expect(find.byKey(const ValueKey<String>('chat-usage-line')), findsNothing);
  }, tags: const <String>['feature_test__tool_context_budget__widget']);
  ...
  testWidgets('inline code leaves the shared selection highlight visible', (
    tester,
  ) async {
    await pump(tester, <TimelineEventDto>[
      event('assistant.delta', <String, dynamic>{
  ...
  testWidgets('one drag selects across every Markdown block of a response', (
    tester,
  ) async {
    final copied = <String>[];
    tester.binding.defaultBinaryMessenger.setMockMethodCallHandler(
    ...
    final second = find.textContaining('second paragraph', findRichText: true);
  ...
  testWidgets('mixed Markdown elements copy as one readable ordered document', (
    tester,
  ) async {
    final copied = <String>[];
    tester.binding.defaultBinaryMessenger.setMockMethodCallHandler(
...
''';
    ...
    final first = find.textContaining('Composite heading', findRichText: true);
    final last = find.textContaining('Closing paragraph.', findRichText: true);
    final gesture = await tester.startGesture(
      tester.getTopLeft(first) + const Offset(1, 1),
      kind: PointerDeviceKind.mouse,
    );
